...
export async function POST(request) {
  ...
  const { data: app, error: fetchErr } = await supabase
    .from('partner_applications').select('*').eq('id', id).single()
  ...
  if (classes.length > 0) {
    const rows = classes.map((cls, i) => ({
      ...
      is_partner:       true,
      is_active:        true,
      duration_minutes: parseInt(cls.duration) || 60,
    ...
    const { error: insertErr } = await supabase.from('enrichment_classes').insert(rows)
