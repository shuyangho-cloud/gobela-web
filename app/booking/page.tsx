...
interface Props {
  searchParams: Promise<{ status?: string; child?: string; program?: string; msg?: string }>
}
...
export default async function BookingPage({ searchParams }: Props) {
  ...
  const configs = {
    ...
    error: {
      ...
      body: msg ?? 'This link may be invalid or expired. Contact hello@gobela.sg for help.',
    },
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Logo */}
        <div style={{ fontSize: '28px', fontWeight: 800, color: '#1a2e4a', letterSpacing: '-0.5px', marginBottom: '4px' }}>
          GoBela
        </div>
        <div style={{ fontSize: '12px', color: '#93c5fd', marginBottom: '32px' }}>
          Partner Portal
        </div>
        ...
        <div style={{
          ...
          margin: '0 auto 20px',
          fontSize: '32px', fontWeight: 900, color: cfg.iconColor,
        }}>
          {cfg.icon}
        </div>

        {/* Title */}
        <h1 style={{ fontSize: '20px', fontWeight: 800, color: '#1a2e4a', marginBottom: '10px' }}>
          {cfg.title}
        </h1>
